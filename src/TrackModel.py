...
class Decan(gym.Env):
    def __init__(self):
        ...
        RayLow = [0 for i in range(raysi)]
        RayHigh = [1 for i in range(raysi)]
        ...
        self.RayCast = []
        angle = 0
        for i in range(raysi):
            #norm_i = i / (raysi - 1)
            #Dist = 100 + (250 - 100) * 4 * norm_i * (1 - norm_i)    
    ...
    def step(self, action, type = "A"):
        ...
        self.Start = (len(self.centered_track)+(self.curCollider - 3))%len(self.centered_track)
        self.End = (self.curCollider + 5)%len(self.centered_track)
        Start = self.Start
        End = self.End
        if Start > End:
            Start = Start - len(self.centered_track)
        ...
        for line in self.RayCast:
            ...
            if Start < 0:
                intersection = line.Collision([self.centered_track[Start: ] + self.centered_track[:End], self.inflated_track[Start: ] + self.inflated_track[:End]])

            else:
                intersection = line.Collision([self.centered_track[Start: End],self.inflated_track[Start: End]])
        ...
        bool_list = [not bool(x) for x in self.CollisionInt]

        done = reduce(lambda x, y: x and y, bool_list)

        for i in range(len(self.CollisionInt)):
            if self.CollisionInt[i] and not done:
    ...
    def reset(self, seed=None, options=None):
        ...
        self.curCollider = 2 #random.randint(1, len(self.centered_track)-1)
        c = TrackGenerator.GoToCenter(self.centered_track[self.curCollider - 1], self.centered_track[self.curCollider], self.inflated_track[self.curCollider - 1], self.inflated_track[self.curCollider])

        self.car = car.Car(c, VM.Vector2(30, 100))
        self.car.rotation = TrackGenerator.Angle(self.centered_track[self.curCollider - 1], self.centered_track[self.curCollider]) 
        self.Checkpoints = [[self.centered_track[i], self.inflated_track[i], False] for i in range(len(self.centered_track))]
        self.Checkpoints[self.curCollider][2] = True
    ...
    def render(self, reward, Obs,fps = 0, mode='human'):
        ...
        if(mode == "Debug"):
            ...
            if Start < 0:
                ...
                l1 = self.inflated_track[Start: ] + self.inflated_track[:End]
                l2 = self.centered_track[Start: ] + self.centered_track[:End]

                for i in range(len(l1) - 1):
                    a = l1[i]
                    b= l1 [i+1]
                    pygame.draw.line(self.screen, (0, 0, 0), (a.x, a.y), (b.x, b.y), 5)
                ...
                l1 = self.inflated_track[Start: End]
                l2 = self.centered_track[Start: End]
